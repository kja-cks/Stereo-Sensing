...
def aruco_display(corners, ids, rejected, image):
    ...
    if len(corners) > 0:
        ...
        for (markerCorner, markerID) in zip(corners, ids):
            ...
            cv2.putText(image, str(markerID),(topLeft[0] - 10), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5 (0, 255, 0), 2)
            print("[Inderence] AruCo marker ID: {}".format(markerID))
    return image
...
def pose_estimation(frame, aruco_dict_type, matrix_coeffiecents, distortion_coefficeients):
    ...
    corners, ids, rejected_img_points =  cv2.aruco.ArucoDetector(gray, aruco_dict_type, parameters)
      #  cameraMatrix=matrix_coeffiecents,
      #  distCoeff=distortion_coefficeients
...
while cap.isOpened():
    ...
    output = pose_estimation(img, "DICT_4X4_250", intrinsic_camera, distortion)
