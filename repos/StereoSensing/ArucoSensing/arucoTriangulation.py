...
def find_depth(xCenter_right, yCenter_right, xCenter_left, yCenter_left, frame_right, frame_left, baseline, f, alpha, corners):
    ...
    x_right = xCenter_right - (width_right/2)
    x_left = xCenter_left - (width_right/2)
    y_right = yCenter_right - (height_right/2)
    y_left = yCenter_left - (height_left/2)

    # Calculate the disparity:
    if len(corners) == 0:
        disparity = 1000000000000000
    else:
